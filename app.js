...
document.getElementById("roll").addEventListener("click", function () {
  if (inPlay) {
    ...
    if (dice === 1) {
      ...
      roundScore += dice;
      document.querySelector(".curr-score>#cs" + (activePlayer + 1)).textContent = roundScore < 10 ? `0${roundScore}` : roundScore;
...
function checkWinner(activePlayer) {
  ...
  if (scores[activePlayer] >= winningScore) {
    document.querySelector(".p" + (activePlayer + 1) + ">h2").textContent = "Winner!";
    document.querySelector(".p" + (activePlayer + 1 == 2 ? 1 : 2) + ">h2").textContent = "Loser!!";
    ...
    document.querySelector(".p" + (activePlayer + 1)).classList.remove("active");
